...
require_once "db.php";
use entites\client;
use entites\client_moral;
use entites\client_non_salarie;
use entites\client_salarie;
use entites\Compte;
use entites\compte_bloque;
use entites\compte_epargne;
use entites\compte_courant;
//use PDO;
...
//use PDOException;
    ...
    
	{
		...
		public function addClientSalarie(Client_salarie $client_salarie)
		{
			//insertion des Infos dans la table client non salarie
			$req = $this->_db->prepare('INSERT INTO client_salarie (nom, 
			prenom, profession,salaire_actuel, nom_entreprise, adresse_entreprise,
			 identifiant_entreprise, numero_CNI, id_client) 
            VALUES(:nom, :prenom, :profession, :salaire_actuel, :nom_entreprise, 
			:adresse_entreprise,
			:identifiant_entreprise,:cni,:id_client)');

			$req->execute(array(
            'nom' => $client_salarie->getNom(),
            'prenom' => $client_salarie->getPrenom(),
		    'profession' => $client_salarie->getProfession(),
			'salaire_actuel' => $client_salarie->getSalaire_actuel(),
			'nom_entreprise' => $client_salarie->getNom_entreprise(),
			'adresse_entreprise' => $client_salarie->getAdresse_entreprise(),
			'identifiant_entreprise' => $client_salarie->getIdentifiant_entreprise(),
            'cni' => $client_salarie->getCni(),
			 'id_client' => $client_salarie->getId_client()
			));

			$req->closeCursor();

//info client salarie depuis la base de donnee
		//	nom`, `prenom`, `profession`, `salaire_actuel`, `nom_entreprise`, 
